...
const getNum = (): Effect<number, GetNumError> => fail(new GetNumError());
...
const getNum = (): Effect<number, ExternalError> => fail(new ExternalError());
...
const program = pipe(
    ...
    matchEffect({
        onFailure: () => fallback(),
        onSuccess: (num) => succeed(num < 0 ? 0 : num),
    }),
    map((num) => num + 1),
    tap((num) => console.log(num))
);
